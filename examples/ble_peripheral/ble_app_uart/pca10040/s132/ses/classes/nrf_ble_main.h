...
// static bool flag_echoback = false;
//
...
//           } break;
//
//           case BLE_ADV_EVT_FAST:{ /** No implementation needed. */ } break;
...
//                     /** Indicate somehow that we stopped scanning */
...
//                     err_code = sd_ble_gap_connect(&p_gap_evt->params.adv_report.peer_addr, &m_scan_params, &m_connection_param, APP_CONN_CFG_TAG);
//
//                     if (err_code != NRF_SUCCESS){
...
//           if (p_ble_evt->header.evt_id != BLE_GAP_EVT_DISCONNECTED){on_ble_central_evt(p_ble_evt);}
...
// */
// static void sys_evt_dispatch(uint32_t evt_id){ble_advertising_on_sys_evt(evt_id);}
//
...
//           NRF_LOG_DEBUG("Data len is set to 0x%X(%d)\r\n", m_ble_nus_max_data_len, m_ble_nus_max_data_len);
//      }
...
//           nrf_drv_rng_bytes_available(&bytes_available);
//           nrf_delay_ms(5);
//           // Return after n attempts.
//           if (retries > 5){ return NRF_ERROR_TIMEOUT;}
//      }
//
//      NRF_LOG_DEBUG("Available random bytes: %d \r\n", bytes_available);
//      err_code = nrf_drv_rng_rand(p_buff, size);
//      RETURN_IF_ERROR(err_code);
//
//      NRF_LOG_DEBUG("Random value (hex): ");
//      for (uint8_t i = 0; i < size; i++){NRF_LOG_DEBUG("%02x", p_buff[i]); }
//      NRF_LOG_DEBUG("\r\n");
//      return NRF_SUCCESS;
// }
//
...
// }
//
// static uint32_t debug_err_code(uint32_t err, const char* func_id){
//      if(err != NRF_SUCCESS){
//           NRF_LOG_ERROR("Function (\'%s\') failed w/ code = %d\r\n",func_id, err);
//           return -1;
//      }
//      return NRF_SUCCESS;
// }
...
//      err = debug_err_code(err_code,"random_vector_generate");
//      if(err < 0){return err_code;}
//
//      err_code = test_gap_params_init();
//      err = debug_err_code(err_code,"gap_params_init");
//      if(err < 0){return err_code;}
//
//      err_code = test_gatt_init();
//      err = debug_err_code(err_code,"gatt_init");
//      if(err < 0){return err_code;}
//
//      err_code = ble_services_init(m_service_handles, m_service_num);
//      err = debug_err_code(err_code,"Thingy_ble_init");
//      if(err < 0){return err_code;}
//
//
//      err_code = advertising_init();
//      err = debug_err_code(err_code,"Advertising_init");
//      if(err < 0){return err_code;}
//
//      err_code = test_conn_params_init();
//      err = debug_err_code(err_code,"Conn_params_init");
//      if(err < 0){return err_code;}
//
//      err_code = ble_advertising_start(BLE_ADV_MODE_FAST);
//      err = debug_err_code(err_code,"ble_advertising_start");
//      if(err < 0){return err_code;}
//      else{NRF_LOG_INFO("BLE Advertising\r\n");}
//
//      err_code = support_func_ble_mac_address_get(m_mac_addr);
//      err = debug_err_code(err_code,"mac address get");
//      if(err < 0){return err_code;}
//
